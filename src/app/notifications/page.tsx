...
import { Bell, Clock, Info, AlertCircle, CheckCircle2, Loader2, Check, ExternalLink, ChevronRight, Sparkles, Zap, MessageSquare } from "lucide-react";
        ...
        <div className="flex justify-between items-center px-4">
          ...
          {notifications && notifications.some(n => !n.isRead) && (
            <Button 
              ...
              className="h-10 rounded-full text-[10px] font-black text-[#002d4d] bg-white border border-gray-100 hover:bg-gray-50 shadow-sm px-6"
            >
          ...
          {isLoading ? (
            ...
            notifications.map((n) => (
              <Card key={n.id} className={cn(
                "border-none shadow-sm rounded-[40px] overflow-hidden transition-all group hover:shadow-md active:scale-[0.99]",
                !n.isRead ? "bg-white ring-2 ring-blue-50" : "bg-white/60 opacity-80"
              )}>
                <CardContent className="p-8 flex items-start gap-6">
                  <div className={cn(
                    "h-16 w-16 rounded-[24px] flex items-center justify-center shrink-0 shadow-inner group-hover:scale-105 transition-transform",
                    !n.isRead ? "bg-blue-50" : "bg-gray-100"
                  )}>
                    {getIcon(n.type)}
                  </div>
                  ...
                  {!n.isRead && (
                    <div className="relative flex h-3 w-3 mt-3 shrink-0">
            ...
            <div className="text-center py-40 bg-white/40 rounded-[56px] border-2 border-dashed border-gray-100 flex flex-col items-center gap-8">
              ...
              <div className="space-y-1">
                <p className="text-gray-300 font-black text-base uppercase tracking-widest">Your Inbox is Clear</p>
