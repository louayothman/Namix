...
/**
 * @fileOverview محرك التداول بداخل الدردشة v1.0 - Telegram Chat Trading Engine
 * يدير الصفقات اللحظية، التحديثات السعرية، والتداول الآلي حصرياً لتلغرام.
 */
...
export async function showChatMarkets(botToken: string, chatId: string, messageId: string) {
  ...
  const text = `📊 *محطة التداول الفوري — الأسواق المتاحة*\n\nاختر الأصل المالي المراد مراقبة نبضه وبدء تنفيذ العمليات:`;
...
export async function toggleChatAutoTrade(botToken: string, chatId: string, messageId: string, currentStatus: boolean) {
  ...
  const text = !currentStatus 
    ...
    : `⚪ *تم إيقاف التداول الآلي*\n\nيمكنك الآن تنفيذ الصفقات يدوياً عبر قائمة الأسواق.`;
...
/**
 * محاكي تنفيذ الصفقة بداخل الدردشة مع التحديث اللحظي
 */
export async function executeChatTrade(botToken: string, chatId: string, symbolId: string, side: 'buy' | 'sell', amount: number, duration: number) {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    await fetch(`https://api.telegram.org/bot${botToken}/editMessageText`, {
      ...
      body: JSON.stringify({
        ...
        text: `⏳ *فتح مركز ${side.toUpperCase()}...*\n[${bars}] ${i * 33}%`,
        parse_mode: 'Markdown'
      })
    });
  ...
  const entryPrice = 64231.50; // محاكاة سعر الدخول
  const endTime = new Date(Date.now() + duration * 1000);
  
  // 4. حلقة التحديث اللحظي (محاكاة لـ 5 تحديثات قبل الإغلاق)
  const totalTicks = 5;
  for (let i = 1; i <= totalTicks; i++) {
    const currentPrice = entryPrice + (side === 'buy' ? (Math.random() * 100 - 20) : (Math.random() * 20 - 100));
    const isWinning = side === 'buy' ? currentPrice > entryPrice : currentPrice < entryPrice;
    const statusEmoji = isWinning ? "🟢" : "🔴";
    const statusText = isWinning ? "ربح مؤقت" : "خسارة مؤقتة";
...
⏳ *متبقي:* ${Math.max(0, Math.floor((endTime.getTime() - Date.now()) / 1000))} ثانية
    `.trim();
