...
const BROADCAST_TEMPLATES = [
  ...
  { id: 'maintenance', label: "صيانة النظام", title: "إشعار: صيانة دورية مجدولة", message: "سنقوم بإجراء صيانة تقنية دورية لتعزيز أداء المنصة غداً. قد تلاحظ توقفاً مؤقتاً في بعض الخدمات لفترة وجيزة. نشكر تفهمكم." },
  { id: 'vault', label: "رفع أرباح الخزنة", title: "أخبار إيجابية: زيادة نسبة العائد على الرصيد الجاري", message: "تقرر رفع نسبة العائد السنوي في الخزنة. الآن، المبالغ غير المستثمرة تنمو بمعدل أسرع بمجرد بقائها في محفظتك المتاحة." },
  { id: 'partnership', label: "توسيع برنامج الشركاء", title: "كن سفيراً للمنصة: رفع عمولات الشبكة", message: "قمنا بتطوير برنامج الشراكة ليمنحك عوائد أفضل. ادعُ شركاء جدد واستفد من نظام العمولات المطور الذي يكافئ نشاطك في توسيع قاعدة المستخدمين." },
  { id: 'welcome', label: "ترحيب بالمستثمر الجديد", title: "أهلاً بك: انطلاق رحلتك في عالم الاستثمار", message: "يسرنا انضمامك إلينا. محفظتك الآن جاهزة لبدء الاستثمار وتفعيل الخطط المتاحة لتوليد الأرباح بذكاء وأمان." },
  { id: 'withdrawal', label: "تحديث بوابات السحب", title: "إشعار مالي: إضافة قنوات سحب رصيد جديدة", message: "استجابة لطلب المستخدمين، قمنا بإدراج بوابات سحب جديدة تضمن سرعة وصول الأرباح. يمكنك الاطلاع على الخيارات المحدثة في صفحة السحب." },
  { id: 'urgent', label: "تنبيه عاجل", title: "إشعار إداري عاجل وهام", message: "يرجى الانتباه إلى التعليمات الجديدة بخصوص توثيق الحسابات. يتطلب النظام إكمال بياناتك لضمان استمرار عمليات السحب دون توقف." },
];
...
export default function AdminNotificationsPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (target === 'single') {
        if (!singleUserId) throw new Error("يرجى تحديد المستخدم المستهدف.");
      ...
      if (targetUsers.length === 0) throw new Error("لم يتم العثور على مستخدمين في هذه الفئة.");
      ...
      toast({ title: "تم الإرسال بنجاح", description: `تم إرسال الرسالة لـ ${targetUsers.length} مستخدم عبر القنوات المختارة.` });
      setFormData({ title: "", message: "", type: "info" });
    } catch (e: any) {
      toast({ variant: "destructive", title: "خطأ في الإرسال", description: e.message });
    } finally {
      setLoading(false);
    }
  };
  ...
  const getTargetLabel = (t: string) => {
    switch(t) {
      ...
      case 'new': return 'المستخدمين الجدد';
      ...
      case 'single': return 'مستخدم فردي';
      default: return t;
    }
  };
      ...
      <div className="max-w-[1600px] mx-auto space-y-10 px-6 pt-10 pb-24 font-body">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-gray-100 pb-8">
          <div className="space-y-2 text-right">
            ...
            <p className="text-muted-foreground font-bold text-xs">إدارة التنبيهات الموجهة وإرسال الرسائل الإدارية عبر المحفظة والبريد الإلكتروني.</p>
          </div>
          ...
          <div className="flex items-center gap-4 bg-gray-50/50 p-2 rounded-full border border-gray-100 shadow-inner">
             <div className="px-6 py-2">
                ...
                <p className="text-lg font-black text-blue-600 tabular-nums">{targetRecipientCount} مستخدم</p>
             </div>
              ...
              <CardContent className="p-10 space-y-10">
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <Input 
                      placeholder="مثال: تحديث أمني هام لنظام السحب" 
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            <Card className="border-none shadow-sm rounded-[48px] bg-[#002d4d] text-white overflow-hidden relative group p-10 space-y-8 text-right">
               ...
               <div className="space-y-2 relative z-10">
                  <h3 className="text-2xl font-black">إدارة الاتصال</h3>
                  <p className="text-[13px] text-white/60 leading-[2.2] font-bold">
                    الإرسال عبر البريد الإلكتروني يضمن توثيق القرارات الإدارية، بينما تضمن إشعارات التطبيق التفاعل اللحظي. استخدم "الإرسال الشامل" للمواثيق القانونية والعروض الهامة.
                  </p>
               </div>
            ...
            <Card className="border-none shadow-sm rounded-[48px] overflow-hidden bg-white group text-right">
              ...
              <CardContent className="p-8 space-y-4">
                {[
                  { label: "المستخدمون النشطون", count: users?.filter(u => (u.activeInvestmentsTotal || 0) > 0).length || 0, color: "bg-emerald-500" },
                  { label: "المستخدمون الجدد (7 أيام)", count: users?.filter(u => new Date(u.createdAt) > new Date(Date.now() - 7*24*60*60*1000)).length || 0, color: "bg-blue-500" },
                  { label: "عمليات معلقة", count: pendingStats.deposits + pendingStats.withdrawals, color: "bg-orange-500" },
                  { label: "إجمالي قاعدة البيانات", count: users?.length || 0, color: "bg-[#002d4d]" }
                ].map((stat, i) => (
