    ...
    try {
      if (portal.automationType === 'nowpayments') {
        ...
        if (res.success) {
          ...
          setError("تعذر توليد الهوية المالية الآلية لهذه العملة.");
        }
      } else {
      ...
      setError("خطأ في مزامنة البروتوكول.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex-1 overflow-y-auto px-5 py-6 scrollbar-none">
            {loadingCats || loading ? (
              <div className="h-full flex flex-col items-center justify-center py-20 gap-4">
                 ...
                 <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">Initializing Secure Node...</p>
              </div>
            ) : (
              <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {categories?.map((cat) => (
                        <button key={cat.id} onClick={() => { setSelectedCatId(cat.id); setStep("select_portal"); }} className="p-6 rounded-[40px] border border-gray-100 bg-white hover:border-[#002d4d] hover:shadow-xl transition-all duration-500 flex flex-col items-center gap-3 active:scale-[0.98] relative overflow-hidden group">
                          <div className="h-12 w-12 rounded-[20px] bg-gray-50 flex items-center justify-center shadow-inner group-hover:bg-[#002d4d] group-hover:text-[#f9a885] transition-all"><Wallet size={24} /></div>
                      ...
                      {activePortals.map((p: any) => (
                        <button key={p.id} onClick={() => handlePortalSelect(p)} className="p-6 rounded-[40px] border border-gray-100 bg-white hover:border-[#002d4d] hover:shadow-xl transition-all duration-500 flex flex-col items-center gap-3 active:scale-[0.98] relative overflow-hidden group">
                          ...
                          <div className="text-center">
                             ...
                             <Badge className={cn("text-[6px] font-black border-none px-1.5 py-0.5 mt-1.5", p.automationType === 'nowpayments' ? "bg-purple-50 text-purple-600" : "bg-blue-50 text-blue-600")}>
                  ...
                  <div className="space-y-8 animate-in zoom-in-95 duration-500 text-right">
                    ...
                    <div className="p-6 bg-gray-50 rounded-[40px] border border-gray-100 shadow-inner space-y-4">
                       <div className="space-y-2">
                          <Label className="text-[9px] font-black text-gray-400 uppercase pr-2 tracking-widest">Your Private Deposit Address</Label>
                    ...
                    {selectedPortal.automationType === 'nowpayments' ? (
                      <div className="p-10 bg-[#002d4d] rounded-[48px] text-center space-y-6 relative overflow-hidden group shadow-2xl">
                         ...
                         <div className="h-16 w-16 rounded-[24px] bg-white/10 flex items-center justify-center backdrop-blur-xl border border-white/20 mx-auto"><Loader2 className="h-8 w-8 animate-spin text-[#f9a885]" /></div>
                         <div className="space-y-2 relative z-10">
                            <h4 className="text-lg font-black text-white">بانتظار التحويل السحابي...</h4>
                            <p className="text-[10px] font-bold text-blue-100/60 px-6 leading-relaxed">سيقوم المفاعل بتحديث رصيدك آلياً بمجرد تأكيد الحوالة على البلوكشين.</p>
                         </div>
                         <Button onClick={handleClose} variant="ghost" className="text-[9px] font-black text-blue-200/40 uppercase tracking-[0.3em] hover:text-white">Close & Monitor</Button>
                      </div>
                    ) : (
                      <div className="space-y-6">
                ...
                {step === "verifying" && (
                  <div className="h-full flex flex-col items-center justify-center py-24 gap-8 animate-in zoom-in-95 text-center">
                     <div className="relative"><div className="h-32 w-32 border-[4px] border-gray-50 border-t-blue-600 rounded-full animate-spin" /><div className="absolute inset-0 flex items-center justify-center"><Cpu size={48} className="text-blue-600 animate-pulse" /></div></div>
                     <div className="space-y-2"><h3 className="text-xl font-black text-[#002d4d]">جاري مطابقة البيانات...</h3><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Binance SAPI Verification in progress</p></div>
                  </div>
                )}
