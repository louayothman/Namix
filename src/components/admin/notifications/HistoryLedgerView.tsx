...
import { History, Clock, Users, Mail, Bell, Globe, Activity, Loader2, Search, Target } from "lucide-react";
...
export function HistoryLedgerView() {
  ...
  const logsQuery = useMemoFirebase(() => query(collection(db, "broadcast_logs"), orderBy("createdAt", "desc"), limit(100)), [db]);
    ...
    <div className="space-y-8 animate-in fade-in duration-700 text-right font-body" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 px-4">
         <div className="relative w-full max-w-md">
            ...
            <Input 
              placeholder="البحث في سجل الحملات..." 
              value={searchTerm}
         ...
         <Badge className="bg-blue-50 text-blue-600 border-none font-black text-[10px] px-6 py-2 rounded-full shadow-inner tracking-widest uppercase">
            {filtered.length} BROADCAST RECORDS
         </Badge>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50/50 border-none hover:bg-gray-50/50">
                <TableHead className="pr-10 py-6 text-[10px] font-black uppercase text-gray-400 tracking-widest text-right">الحملة</TableHead>
                <TableHead className="text-[10px] font-black uppercase text-gray-400 tracking-widest text-right">القناة</TableHead>
                <TableHead className="text-[10px] font-black uppercase text-gray-400 tracking-widest text-right">الفئة</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow><TableCell colSpan={5} className="text-center py-32"><Loader2 className="h-8 w-8 animate-spin mx-auto text-gray-200" /></TableCell></TableRow>
              ) : filtered.length > 0 ? (
                filtered.map((log) => (
                  <TableRow key={log.id} className="hover:bg-gray-50/30 transition-all border-gray-50 group">
                    <TableCell className="pr-10 py-6">
                       <div className="flex items-center gap-4">
                          ...
                          <div className="text-right">
                             ...
                             <p className="text-[9px] font-bold text-gray-400 truncate max-w-[200px]">{log.message || "محتوى مخصص"}</p>
                          </div>
                       </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-gray-500 uppercase">{log.target || 'General'}</span>
                       </div>
                    </TableCell>
                    <TableCell>
                       <div className="flex items-center gap-2">
                ...
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-48 opacity-20 flex flex-col items-center gap-4">
