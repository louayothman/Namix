...
/**
 * @fileOverview مركز هندسة بوابات الإيداع الشمولية v9.0
 * تم تحديث القائمة لتشمل بوابة التحويل الداخلي (Namix Internal) السيادية.
 */
              ...
              <CardContent className="p-10 space-y-8 border-t border-gray-50">
                 <div className="grid gap-8 md:grid-cols-2">
                    <div className="space-y-6">
                       ...
                       <div className="space-y-2">
                          <Label className="text-[10px] font-black text-gray-400 pr-4 uppercase tracking-widest">نمط التشغيل</Label>
                    ...
                    <div className="space-y-2">
                       ...
                       <Textarea value={newCat.description} onChange={e => setNewCat({...newCat, description: e.target.value})} className="min-h-[140px] rounded-[32px] bg-gray-50 border-none font-bold text-xs p-6 shadow-inner leading-relaxed" placeholder="اكتب وصفاً يظهر للمستثمر..." />
                    </div>
                 </div>
...
function CategoryEditor({ category, onBack }: { category: any, onBack: () => void }) {
  ...
  const [newPortal, setNewPortal] = useState({ name: "Namix Internal Transfer", walletAddress: "", instructions: "", icon: "NAMIX_ID" });
  ...
  const addPortal = async () => {
    ...
    try {
      const portalData = {
        ...
        ...newPortal,
        isActive: true,
        fields: [{ label: "Namix ID المرسل", placeholder: "أدخل ID المستخدم الذي قام بالتحويل...", type: "text", hasPasteButton: true }]
      };
      await updateDoc(doc(db, "deposit_methods", category.id), {
        portals: arrayUnion(portalData)
      });
      setNewPortal({ name: "Namix Internal Transfer", walletAddress: "", instructions: "", icon: "NAMIX_ID" });
               ...
               <CardContent className="p-8 space-y-8">
                  <div className="grid grid-cols-2 gap-6">
                     <div className="space-y-2">
                        ...
                        <Input value={data.name} onChange={e => setData({...data, name: e.target.value})} className="h-12 rounded-xl bg-gray-50 border-none font-black px-8 shadow-inner" />
                     </div>
                     <div className="space-y-2">
                 ...
                 <CardContent className="p-8 space-y-8">
                    <div className="grid grid-cols-2 gap-6">
                       <div className="space-y-2">
                          ...
                          <Input value={newPortal.name} onChange={e => setNewPortal({...newPortal, name: e.target.value})} className="h-12 rounded-xl bg-gray-50 border-none font-black px-6" placeholder="Namix Internal Transfer" />
                       </div>
                       <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                       <Label className="text-[10px] font-black text-gray-400 pr-4 uppercase">عنوان الاستلام (الـ ID الخاص بك أو محفظة)</Label>
                       <Input value={newPortal.walletAddress} onChange={e => setNewPortal({...newPortal, walletAddress: e.target.value})} className="h-12 rounded-xl bg-gray-50 border-none font-mono text-[10px] font-black px-6 text-left" dir="ltr" placeholder="أدخل ID الاستلام المركزي..." />
                    </div>
                    <div className="space-y-2">
                       <Label className="text-[10px] font-black text-gray-400 pr-4 uppercase">تعليمات الدفع للمستثمر</Label>
                       <Textarea value={newPortal.instructions} onChange={e => setNewPortal({...newPortal, instructions: e.target.value})} className="min-h-[100px] rounded-[24px] bg-gray-50 border-none font-bold text-[10px] p-6 leading-loose" placeholder="يرجى تحويل المبلغ لـ ID ناميكس الموضح..." />
                    </div>
