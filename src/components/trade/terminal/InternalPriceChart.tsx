...
export function InternalPriceChart({ asset, livePrice }: InternalPriceChartProps) {
  ...
  return (
    <div className="w-full h-full relative bg-[#fcfdfe] overflow-hidden font-body">
      ...
      <div className="absolute top-4 left-4 z-[100] flex justify-start pointer-events-none">
        <div className="flex flex-col items-start gap-3">
          <div className="flex items-center gap-2 pointer-events-auto">
            ...
            <div className="h-7 px-3 bg-white/90 border border-gray-100 rounded-full flex items-center gap-2 shadow-lg backdrop-blur-md">
               <span className="text-[9px] font-black text-[#002d4d] flex items-center gap-1.5"><Sparkles size={10} className="text-[#f9a885]"/> NAMIX INTERNAL</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {priceY !== null && (
          <motion.div initial={false} animate={{ y: priceY - 12 }} transition={{ type: "spring", stiffness: 450, damping: 35 }} className="absolute right-0 z-[120] pointer-events-none" style={{ width: '100%' }}>
            <div className={cn("absolute right-[70px] top-[12px] h-[1.5px] w-[200vw] border-t border-solid transition-colors duration-500 opacity-40 shadow-[0_0_10px_currentColor]", isUp ? "border-emerald-400 text-emerald-400" : "border-red-400 text-red-400")} />
      ...
      {isLoading && <div className="absolute inset-0 z-[110] bg-white/60 flex flex-col items-center justify-center gap-4"><Loader2 className="h-10 w-10 animate-spin text-[#002d4d]" /><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">معايرة الرسم الداخلي...</p></div>}
