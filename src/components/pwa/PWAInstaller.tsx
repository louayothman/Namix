...
/**
 * @fileOverview مُفاعل التثبيت الذكي v1.0
 * يدير عملية تحويل الموقع إلى تطبيق (PWA) عبر نوافذ مخصصة للأندرويد وإرشادات للآيفون.
 */
...
export function PWAInstaller() {
  ...
  useEffect(() => {
    ...
    // 2. معالجة تثبيت أندرويد/كروم
    const handleBeforeInstallPrompt = (e: Event) => {
      e.preventDefault();
      setDeferredPrompt(e);
      // إظهار المحفز بعد 3 ثوانٍ من دخول الصفحة لضمان تجربة هادئة
      setTimeout(() => setShowAndroidModal(true), 3000);
    };

    window.addEventListener('beforeinstallprompt', handleBeforeInstallPrompt);

    // 3. معالجة آيفون (Safari)
    const isIos = /iPad|iPhone|iPod/.test(navigator.userAgent) && !(window as any).MSStream;
    if (isIos && !isStandalone) {
      setTimeout(() => setShowIosTooltip(true), 4000);
    }
          ...
          <div className="fixed inset-0 z-[2000] flex items-end md:items-center justify-center p-4 md:p-6 pointer-events-none font-body">
            ...
            >
              ...
              <button 
                ...
                className="absolute left-6 top-6 h-8 w-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-300 hover:text-red-500 transition-colors"
              >
                <X size={16} />
              </button>
              ...
              <div className="space-y-6 relative z-10">
                <div className="flex items-center gap-4">
                   <div className="h-14 w-14 rounded-2xl bg-[#002d4d] flex items-center justify-center shadow-xl shadow-blue-900/20">
                      <Download size={24} className="text-[#f9a885] animate-bounce" />
                   </div>
                   <div className="text-right">
                      <h3 className="text-lg font-black text-[#002d4d] tracking-normal">ثبّت تطبيق ناميكس</h3>
                ...
                <p className="text-[12px] font-bold text-gray-500 leading-loose text-right">
                  انتقل لتجربة ناميكس الكاملة؛ ثبّت التطبيق الآن للوصول السريع وحماية أصولك بضغطة واحدة من شاشتك الرئيسية.
                </p>

                <div className="grid grid-cols-1 gap-3">
                   <Button 
                     onClick={handleAndroidInstall}
                     className="w-full h-14 rounded-full bg-[#002d4d] hover:bg-[#001d33] text-white font-black text-sm shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3 group"
                   >
                   ...
                   <button 
                     onClick={() => setShowAndroidModal(false)}
                     className="w-full h-10 text-[10px] font-black text-gray-300 uppercase tracking-widest hover:text-[#002d4d] transition-colors"
                   >
                     سأقوم بذلك لاحقاً
                   </button>
                </div>
        ...
        {showIosTooltip && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[2000] w-[90vw] max-w-[360px] pointer-events-none font-body" dir="rtl">
            <motion.div 
              ...
              exit={{ y: 20, opacity: 0 }}
              className="bg-[#002d4d] text-white p-6 rounded-[32px] shadow-2xl relative pointer-events-auto border border-white/10"
            >
              {/* Pointer Triangle */}
              ...
              <button 
                ...
                className="absolute left-4 top-4 opacity-40 hover:opacity-100 transition-opacity"
              >
                <X size={14} />
              </button>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                   ...
                   <p className="text-sm font-black tracking-normal">أضف ناميكس لشاشتك</p>
                </div>
                ...
                <div className="space-y-3">
                   <p className="text-[11px] font-bold text-blue-100/70 leading-relaxed text-right">
                     ...
                     <PlusSquare size={14} className="inline-block mx-1 text-[#f9a885]" /> لتجربة تداول غامرة.
                   </p>
                </div>
