...
export function PrivacyDialog({ open, onOpenChange }: PrivacyDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      fetch("https://lottie.host/dfea4986-b9c6-488e-9273-02ae186dc594/Sva0KM402u.json")
        .then(res => res.json())
            ...
            <div className="w-1/2 p-5 md:p-16 space-y-6 md:space-y-12 bg-white relative flex flex-col justify-center">
               ...
               {isLoading ? (
                 <div className="py-10 flex flex-col items-center gap-4 text-center">
                    ...
                    <p className="text-[8px] font-black text-gray-300 uppercase tracking-widest">جاري جلب ميثاق الخصوصية...</p>
                 </div>
               ) : (
                 <div className="space-y-5 md:space-y-10 animate-in fade-in slide-in-from-left-4 duration-1000">
                    ...
                    <div className="max-h-[150px] md:max-h-[250px] overflow-y-auto pr-1 scrollbar-none">
                      <p className="text-gray-500 font-bold leading-[1.8] md:leading-[2.2] text-[10px] md:text-sm text-right whitespace-pre-wrap">
                         {legal?.privacyPolicy || "نحن في ناميكس نولي خصوصية بياناتك أهمية قصوى. يتم تشفير كافة المعلومات الشخصية والمالية وفقاً لأعلى المعايير الدولية لضمان أمان استثماراتك وخصوصية هويتك الرقمية."}
                      </p>
                    </div>

                    <div className="grid grid-cols-3 gap-2 md:gap-4">
                       {[
                         { title: "تشفير AES-256", color: "text-emerald-600", bg: "bg-emerald-50" },
                         { title: "سرية مطلقة", color: "text-blue-600", bg: "bg-blue-50" },
