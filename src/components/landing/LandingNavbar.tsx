...
/**
 * @fileOverview شريط الملاحة العالمي v6.1 - Integrated White Theme
 * الشريط العلوي مدمج بالكامل مع الخلفية البيضاء، مع عكس اتجاه العناصر في الجوال.
 */
...
export function LandingNavbar() {
  ...
  return (
    <nav className="absolute top-0 left-0 right-0 z-[1000] w-full bg-[#7D8E9E] py-6 md:py-8 px-4 md:px-12 flex items-center justify-between select-none border-none shadow-none">
      ...
      {/* 1. Identity Node (Right in RTL / Left in Visual) - Grid on Right, Text on Left */}
      ...
      <div className="flex-[2] flex items-center justify-center gap-6 md:gap-12" dir="rtl">
        {navLinks.map((link) => (
          ...
          >
            <span className="text-[11px] md:text-[14px] font-black text-white group-hover:text-[#f9a885] transition-all tracking-normal whitespace-nowrap">
              {link.name}
            </span>
      ...
      {/* 3. Action Node (Left in RTL / Right in Visual) */}
      <div className="flex-1 flex justify-end">
        <Link href={isLoggedIn ? "/home" : "/login"}>
          <Button className="h-11 md:h-14 px-8 md:px-12 rounded-full bg-[#f9a885] hover:bg-white text-[#002d4d] font-black text-[11px] md:text-[13px] shadow-xl active:scale-95 transition-all group overflow-hidden relative border-none">
