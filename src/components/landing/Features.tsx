      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center space-y-4 mb-24">
          ...
          >
            ...
            <p className="text-gray-400 font-bold text-sm uppercase tracking-[0.3em]">Professional Performance Protocol</p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {featureList.map((f, i) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className="relative p-12 rounded-[56px] bg-gray-50/30 border border-gray-100 hover:bg-white hover:shadow-[0_40px_80px_-15px_rgba(0,45,77,0.1)] transition-all duration-700 group overflow-hidden"
            >
              ...
              <motion.div
                ...
                className={cn(
                  "absolute -bottom-6 -left-6 opacity-[0.03] group-hover:opacity-[0.08] transition-all duration-1000 pointer-events-none",
                  f.color
                )}
              >
                <f.icon size={180} strokeWidth={1.5} />
              </motion.div>

              {/* Decorative Accent */}
              <div className={cn(
                "absolute top-12 right-12 w-1 h-1 rounded-full animate-pulse shadow-[0_0_10px_currentColor]",
                f.color
              )} />

              <div className="relative z-10 space-y-6 text-right">
                <h3 className="text-2xl font-black text-[#002d4d] tracking-tight group-hover:text-blue-600 transition-colors duration-500">
                  {f.title}
                </h3>
                <p className="text-gray-500 font-medium leading-[2.2] text-sm md:text-base opacity-80 group-hover:opacity-100 transition-opacity">
                  {f.desc}
                </p>
