...
/**
 * @fileOverview مُفاعل الجسيمات والشبكة العصبية v1.1 - Hydration Fix
 * يولد نقاطاً تسبح بحركة عشوائية متصلة بخطوط بروتوبلازمية نحيفة جداً.
 * تم تأجيل توليد النقاط لما بعد الهيدريشن لتجنب أخطاء SSR.
 */
export function MarketPulseBackground() {
  const [points, setPoints] = useState<{id: number, x: number, y: number, duration: number}[]>([]);

  useEffect(() => {
    // توليد النقاط في بيئة العميل فقط لضمان تطابق الهيدريشن
    const generatedPoints = Array.from({ length: 25 }).map((_, i) => ({
      id: i,
      x: Math.random() * 100,
      y: Math.random() * 100,
