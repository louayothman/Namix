...
export function DiceReactor({ lastResult, gameState, isRollOver, targetValue, setTargetValue, setIsRollOver }: DiceReactorProps) {
  return (
    <section className="flex-1 flex flex-col items-center justify-center p-6 space-y-12 font-body">
      ...
      <div className="relative w-full max-w-[260px] space-y-10">
        <div className="relative pt-12">
          ...
          <div className="relative mt-2">
             ...
             {/* السهم الرقمي الطائر */}
             <motion.div 
               className="absolute top-[-48px] flex flex-col items-center pointer-events-none" 
               style={{ left: `${targetValue}%`, transform: 'translateX(-50%)' }}
               animate={{ left: `${targetValue}%` }}
             >
                <div className="bg-[#002d4d] text-white px-3 py-1 rounded-xl text-[10px] font-black shadow-2xl flex items-center gap-1 border border-white/10 tabular-nums shadow-[0_0_15px_rgba(59,130,246,0.3)]">
          ...
          <div className="flex justify-between items-center mt-6 font-black text-[8px] text-gray-300 uppercase tracking-widest px-1">
            <span>0.00</span>
            <span className="opacity-30">CENTRAL</span>
            <span>100.00</span>
          </div>
        </div>
        ...
        <div className="flex justify-center">
          <div className="flex items-center gap-1 p-1 bg-gray-50 rounded-2xl border border-gray-100 shadow-inner">
            <button onClick={() => setIsRollOver(true)} className={cn("px-6 h-10 rounded-xl font-black text-[10px] transition-all", isRollOver ? "bg-[#002d4d] text-[#f9a885] shadow-lg" : "text-gray-400")}>Over</button>
            <button onClick={() => setIsRollOver(false)} className={cn("px-6 h-10 rounded-xl font-black text-[10px] transition-all", !isRollOver ? "bg-[#002d4d] text-[#f9a885] shadow-lg" : "text-gray-400")}>Under</button>
          </div>
        </div>
      </div>
    </section>
