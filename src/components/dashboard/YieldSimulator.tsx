...
/**
 * ResultNode - مكوّن نانوي لعزل مخرجات المحاكي
 */
const ResultNode = memo(({ label, value, colorClass }: { label: string, value: string, colorClass: string }) => (
  <div className="p-3 bg-gray-50/50 rounded-[24px] border border-gray-100 shadow-inner flex flex-col items-center text-center space-y-0.5 gpu-accelerated">
    <p className="text-[7px] font-black text-gray-400 uppercase tracking-widest leading-none">{label}</p>
    <div className={cn("flex items-center text-xl font-black tabular-nums tracking-tighter h-[1.2em] leading-none", colorClass)} dir="ltr">
...
export function YieldSimulator({ marketingConfig, calcAmount, onAmountChange, onIncrement, onDecrement }: YieldSimulatorProps) {
  ...
  return (
    <div className="relative py-1 font-body tracking-normal select-none" dir="rtl">
      <Card className="border-none shadow-xl rounded-[40px] bg-white overflow-hidden border border-gray-100 relative group flex flex-col max-w-2xl mx-auto gpu-accelerated">
        ...
        <div className="px-6 py-3 grid grid-cols-2 gap-3 relative z-10 border-b border-gray-50">
           <ResultNode label="التدفق الشهري" value={monthlyProfit.toLocaleString()} colorClass="text-blue-600" />
           <ResultNode label="العائد السنوي" value={annualProfit.toLocaleString()} colorClass="text-emerald-600" />
        </div>
           ...
           <div className="max-w-sm mx-auto space-y-4">
              <div className="text-center space-y-1.5">
                 ...
                 <div className="flex items-center justify-center gap-4">
                    <button onClick={onDecrement} className="h-9 w-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-red-500/20 transition-all active:scale-90 outline-none">
                       <Minus size={14} />
                    </button>
                    
                    <div className="flex items-center text-3xl font-black tabular-nums tracking-tighter h-[1.2em] leading-none select-none text-white" dir="ltr">
                       <span className="text-lg font-bold text-[#f9a885] mr-1">$</span>
                       <span>{amountValue.toLocaleString()}</span>
                    </div>

                    <button onClick={onIncrement} className="h-9 w-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#f9a885] hover:text-[#002d4d] transition-all active:scale-90 outline-none">
                       <Plus size={14} />
                    </button>
                 </div>
              </div>
              ...
              <div className="flex flex-row items-center justify-between gap-3">
                 ...
                 <Button onClick={() => router.push('/invest')} className="h-10 px-6 rounded-full bg-[#f9a885] hover:bg-white text-[#002d4d] font-black text-[9px] shadow-xl active:scale-95 transition-all group">
                    بدء التشغيل
